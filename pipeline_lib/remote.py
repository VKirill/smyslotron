"""Удалённая сборка HAC-деревьев на GPU Vast.ai, когда локальной RAM не хватает.

Система сама: оценивает нужную VRAM (float32, иначе float16), ищет самую дешёвую
верифицированную карту, поднимает её, заливает вектора представителей (.npy),
строит деревья NN-chain-алгоритмом на CUDA (gpu_hac.py), забирает домой только
.bin + пороги и уничтожает машину. Всегда — даже при ошибке.
Замер на 104 602 смыслах: H200 $3.32/ч — весь цикл 7 мин, $0.39.

Env: VASTAI_API_KEY (обязателен), VASTAI_GPU_MAX_DPH (макс $/час, дефолт 4.0),
...
# проверенная схема из seo-cluster: PyTorch-образ + встроенный шаблон Vast.ai (sshd, torch, numpy)
IMAGE = "pytorch/pytorch:2.5.1-cuda12.4-cudnn9-runtime"
TEMPLATE_HASH = "f88cf873a299c0863309bab988ad115c"
# ступени VRAM под реальные карты: 4090 / A6000-L40 / A100-H100 / RTX PRO 6000 / H200
VRAM_STEPS_GB = (24, 48, 80, 94, 140)
...
class Vast:
    def __init__(self, key: str):
        self.c = httpx.AsyncClient(timeout=60, headers={"Authorization": f"Bearer {key}"})
...
async def build_remote(emb: np.ndarray, provider: str, data_dir: Path,
    ...
    """Строит деревья провайдера на арендованной GPU. Возвращает (t_fine, t_coarse)."""
    key = os.environ.get("VASTAI_API_KEY")
    if not key:
        raise MemoryError("локальной RAM не хватает, а VASTAI_API_KEY не задан — "
                          "удалённая сборка на GPU недоступна")
    n = emb.shape[0]
    max_dph = float(os.environ.get("VASTAI_GPU_MAX_DPH", "4.0"))
    vast = Vast(key)
    vram, prec = pick_vram_step(n)
    needv = need_vram_gb(n, f16=(prec == "f16"))
    set_status(f"Vast.ai GPU: ищу карту с VRAM ≥ {vram} ГБ ({prec}, нужно ~{needv:.0f} ГБ "
               f"для {n:,} смыслов)", 40)
    ...
    try:
        ...
        for _ in range(30):  # ждём, пока sshd примет ключ
            try:
        ...
        set_status("Vast.ai GPU: проверяю torch, заливаю вектора…", 45)
        await _sh("ssh", *base, f"root@{host}",
                  "mkdir -p /work && python -c 'import numpy, torch'", timeout=600)
        tmp = data_dir / f"_reps_{provider}.npy"
        np.save(tmp, np.asarray(emb, dtype=np.float32))
        try:
            await _sh("scp", *_scp_base(host, port), str(tmp), f"root@{host}:/work/reps.npy",
                      timeout=3600)
        finally:
        ...
        thr = json.loads((await _sh("ssh", *base, f"root@{host}", "cat /work/thr.json",
                                    timeout=60)).decode())
        for name in ("hard", "soft", "avg"):  # валидация: n-1 слияний
