...
11. **Глобальная база эмбеддингов `embeddings.db`** (`pipeline_lib/store.py`): фраза оплачивается один раз навсегда, `EmbStore._embed` сперва читает базу, платит только за промахи. Не удалять и бэкапить — это самое дорогое, что есть.
12. **Автоаренда GPU Vast.ai (`pipeline_lib/remote.py` + `gpu_hac.py`)** включается сама, когда матрица не влезает в MemAvailable. Инстанс ВСЕГДА уничтожается в `finally`; перед рестартом бэкенда проверять `SELECT … status='running'` — убитый прогон = потерянная аренда. `scp -P`, `ssh -p`; ключ в аккаунте Vast.ai до создания; endpoint `public_ipaddr + ports["22/tcp"]`. Проверка после правок — ТОЛЬКО на полном рабочем файле (правило пользователя, тесты на мелких проектах не засчитываются); разбиения avg/soft должны совпасть со scipy, hard ≥ 99%.
13. **Частотность — НЕ критерий мусора.** Ноль в колонке частотности — это ноль в сервисе, откуда снимали статистику, а не «пустой запрос». Никаких автоматических отсечек/фильтров по частоте в пайплайне (нулевики участвуют в эмбеддингах, дедупе и деревьях наравне со всеми). Клиентский фильтр в просмотрщике — единственное допустимое место, и только по явному действию пользователя.
14. **Удалённая сборка — только GPU** (`remote.py` + `gpu_hac.py`, NN-chain на CUDA). CPU-путь через RAM-машины удалён намеренно: на 104К смыслов H200 — 7 мин/$0.39, CPU 503 ГБ — >70 мин.
15. **Шаблоны маппинга глобальные:** `mapping_templates(sig=sha1 первой строки → mapping)`, пишутся при каждом подтверждении маппинга (create/append), применяются в `/uploads` для любого пользователя. Шаблон ТОЛЬКО подставляет колонки в модалку — окно подтверждения показывается всегда, тихого автодобавления нет (пользователь запретил). Мультизагрузка в append шлёт `defer_run: true` на каждый файл и один `/run` в конце — иначе воркер стартует между файлами и второй append упрётся в running.
